...
$labels = @('_bvrCounters:','_bvrEvtRing:','_bvrAbiMagic:','_bvrSnapshotSeq:','_bvrActiveMask:','_bvrDesiredMask:','_bvrMaskEpoch:','_bvrActivationFrame:','_bvrEyePhase:','_bvrFaultFlags:','_bvrTelemetryLevel:','_bvrLastControlEvent:','_bvrClearShouldRequest:','_bvrClearFlagsBefore:','_bvrClearStagingBefore:','_bvrClearFillBefore:','_bvrStereoRequestMode:','_bvrStereoRequestNonce:','_bvrStereoRequestGeneration:','_bvrStereoRequestFlags:','_bvrStereoMatrixSeq:','_bvrStereoForkMagic:','_bvrStereoAckGeneration:','_bvrStereoMatrices:','_bvrStereoRightCamera:','_bvrStereoRightProjection:')
...
$versionLine = $text[$abiStart + 3]
if ($versionLine -notmatch '^\.int 3$') { throw "ABI version must be 3: $versionLine" }
$sizeLine = ($text | Select-String -Pattern '^\.int 0x00000430$').Line
if (-not $sizeLine) { throw 'ABI size must remain 0x430' }
...
if (-not ($text -contains '0x039A9524 = bla hook_bvrBuildNativeRightView')) { throw 'Native view-1 bake hook is missing' }
Write-Output "BVR ABI v3 validation: PASS (ring=$ringInts words, size=0x430, native view-1 hook present, 10 slot-before-index event publishers, all hooks use activeMask)"
