...
$labels = @('_bvrCounters:','_bvrEvtRing:','_bvrAbiMagic:','_bvrSnapshotSeq:','_bvrActiveMask:','_bvrDesiredMask:','_bvrMaskEpoch:','_bvrActivationFrame:','_bvrEyePhase:','_bvrFaultFlags:','_bvrTelemetryLevel:','_bvrLastControlEvent:')
...
$sizeLine = ($text | Select-String -Pattern '^\.int 0x00000160$').Line
if (-not $sizeLine) { throw 'ABI size must remain 0x160' }
...
if ($publishLines.Count -ne 9) { throw "Expected 9 event publishers, found $($publishLines.Count)" }
foreach ($publisher in $publishLines) {
    $index = $publisher.LineNumber - 1
    $window = $text[[Math]::Max(0,$index-5)..($index-1)] -join "`n"
    if ($window -notmatch 'stw r11, 0\(r12\)') { throw "Event index published before its ring slot near line $($publisher.LineNumber)" }
}
Write-Output "BVR2 ABI validation: PASS (ring=$ringInts words, size=0x160, 9 slot-before-index event publishers, all hooks use activeMask)"
