...
bool GameProfile::Load(uint64_t title_id)
{
	...
	while (iniParser.NextSection())
	{
		...
		else if (boost::iequals(iniParser.GetCurrentSectionName(), "Graphics"))
		{
			...
			gameProfile_loadBooleanOption2(iniParser, "disableGPUFence", m_disableGPUFence);
			gameProfile_loadBooleanOption2(iniParser, "aggressiveAsyncPipeline", m_aggressiveAsyncPipeline);
			gameProfile_loadBooleanOption2(iniParser, "disableDrawDoneSync", m_disableDrawDoneSync);
			gameProfile_loadBooleanOption2(iniParser, "enableTraceCache", m_enableTraceCache);
#if ENABLE_METAL
		...
		else if (boost::iequals(iniParser.GetCurrentSectionName(), "CPU"))
		{
			...
			gameProfile_loadIntegerOption(iniParser, "flipQueueDepth", m_flipQueueDepth, 1U, 16U);
...
#define WRITE_ENTRY_NUMBERED(__NAME, __NUM) fs->writeLine(fmt::format("{} = {}", #__NAME #__NUM, m_##__NAME).c_str());
	...
	WRITE_ENTRY(flipQueueDepth);
	fs->writeLine("");

	fs->writeLine("[Graphics]");
	WRITE_ENTRY(accurateShaderMul);
	WRITE_ENTRY(disableGPUFence);
	WRITE_ENTRY(aggressiveAsyncPipeline);
	WRITE_ENTRY(disableDrawDoneSync);
	WRITE_ENTRY(enableTraceCache);
#if ENABLE_METAL
...
void GameProfile::ResetOptional()
{
	...
	m_disableGPUFence = false;
	m_aggressiveAsyncPipeline = false;
	m_disableDrawDoneSync = false;
	m_enableTraceCache = false;
#if ENABLE_METAL
	...
	// gpu settings
	m_flipQueueDepth = kFlipQueueDepthDefault;
	// audio
