...
namespace GX2
{
	...
	{
		// Skip GPU fence if disabled in game profile (reduces CPU/GPU sync stalls)
		if (g_current_game_profile->IsGPUFenceDisabled())
			return;
	...
	void GX2DrawDone()
	{
		// Skip full sync if disabled in game profile (reduces CPU/GPU sync overhead)
		if (g_current_game_profile->IsDrawDoneSyncDisabled())
		{
			// Still flush the pipeline but skip the sync wait
