...
struct
{
	...
	std::mutex recompilerWakeMutex;
	std::condition_variable recompilerWakeCondVar;
}PPCRecompilerState;

// mutex to serialize actual recompilation (the heavy work) - prevents race conditions in recompileFunction
static std::mutex s_recompilationMutex;
...
void PPCRecompiler_visitAddressNoBlock(uint32 enterAddress)
{
	...
	PPCRecompilerState.recompilerSpinlock.unlock();
	// wake the recompiler threads immediately instead of waiting for 10ms poll
	PPCRecompilerState.recompilerWakeCondVar.notify_one();
}
...
{
	// Record hit for trace cache (prewarm optimization)
	PPCTraceCache::GetInstance().RecordHit(hCPU->instructionPointer);
...
std::vector<std::thread> s_recompilerThreads;
std::atomic_bool s_recompilerThreadStopSignal{false};

void PPCRecompiler_thread(int threadIndex)
{
	SetThreadName(fmt::format("PPCRecompiler{}", threadIndex).c_str());
#if PPCREC_FORCE_SYNCHRONOUS_COMPILATION
	return;
#endif
	...
	while (true)
	{
        if(s_recompilerThreadStopSignal)
            return;
		// wait for work with condition variable (immediate wake) with 10ms timeout fallback
		{
			std::unique_lock<std::mutex> lk(PPCRecompilerState.recompilerWakeMutex);
			PPCRecompilerState.recompilerWakeCondVar.wait_for(lk, std::chrono::milliseconds(10));
		}
		...
		while (true)
		{
			...
			// serialize actual recompilation to prevent race conditions
			{
				std::lock_guard<std::mutex> recompileLock(s_recompilationMutex);
				PPCRecompiler_recompileAtAddress(enterAddress);
			}
			if(s_recompilerThreadStopSignal)
				return;
...
uint32 PPCRecompiler_getNumRecompilerThreads()
{
	...
	if (cpuCoreCount <= 2)
		...
		return std::min(cpuCoreCount - 2, 8u); // leave 2 cores for main emulation, cap at 8 recompiler threads
}
...
void PPCRecompiler_init()
{
	...
	// Initialize CPU topology detection for hybrid CPU support
	auto& topology = CpuTopology::GetInstance();
	topology.Initialize();
	cemuLog_log(LogType::Force, "{}", topology.GetTopologyDescription());

	cemuLog_log(LogType::Force, "Recompiler initialized");

	ppcRecompilerEnabled = true;

	// launch recompilation threads
    s_recompilerThreadStopSignal = false;
    uint32 numThreads = PPCRecompiler_getNumRecompilerThreads();
    cemuLog_log(LogType::Force, "Launching {} PPC recompiler thread(s)", numThreads);
    s_recompilerThreads.reserve(numThreads);
    for (uint32 i = 0; i < numThreads; i++)
    {
        s_recompilerThreads.emplace_back(PPCRecompiler_thread, i);
    }

	// On hybrid CPUs, let recompiler threads use all cores except the 3 P-cores reserved for scheduler
	if (topology.IsHybridCPU())
	{
		const auto& pCores = topology.GetPerformanceCores();
		const auto& eCores = topology.GetEfficiencyCores();

		// Build mask: all E-cores + P-cores starting from index 6 (skip first 3 pairs for scheduler)
		uint64_t recompilerMask = 0;
		for (uint32_t core : eCores)
			recompilerMask |= (1ULL << core);
		// Add remaining P-cores (after the ones used for scheduler threads)
		for (size_t i = 6; i < pCores.size(); i++)
			recompilerMask |= (1ULL << pCores[i]);

		if (recompilerMask != 0)
		{
			for (auto& thread : s_recompilerThreads)
			{
				CpuTopology::SetThreadAffinity(thread, recompilerMask);
			}
			cemuLog_log(LogType::Force, "Recompiler threads set to use E-cores and remaining P-cores (mask: 0x{:X})", recompilerMask);
		}
	}

	// Initialize trace cache for speculative prewarming
	PPCTraceCache::GetInstance().Initialize(CafeSystem::GetForegroundTitleId());

	// Prewarm: queue hot addresses from previous runs
	if (PPCTraceCache::GetInstance().IsPrewarmEnabled())
	{
		auto prewarmAddresses = PPCTraceCache::GetInstance().GetPrewarmAddresses();
		if (!prewarmAddresses.empty())
		{
			cemuLog_log(LogType::Force, "[TraceCache] Prewarming {} hot functions...", prewarmAddresses.size());

			// First pass: allocate memory ranges for all prewarm addresses
			for (uint32_t addr : prewarmAddresses)
			{
				if (addr < PPC_REC_CODE_AREA_SIZE && (addr & 3) == 0)
					PPCRecompiler_allocateRange(addr, 4);
			}

			// Second pass: queue addresses for compilation
			PPCRecompilerState.recompilerSpinlock.lock();
			uint32_t queuedCount = 0;
			for (uint32_t addr : prewarmAddresses)
			{
				// Validate address is within code area and 4-byte aligned
				if (addr >= PPC_REC_CODE_AREA_SIZE || (addr & 3) != 0)
					continue;
				// Only queue if not already compiled
				if (ppcRecompilerInstanceData->ppcRecompilerDirectJumpTable[addr / 4] == PPCRecompiler_leaveRecompilerCode_unvisited)
				{
					PPCRecompilerState.targetQueue.emplace(addr);
					ppcRecompilerInstanceData->ppcRecompilerDirectJumpTable[addr / 4] = PPCRecompiler_leaveRecompilerCode_visited;
					queuedCount++;
				}
			}
			PPCRecompilerState.recompilerSpinlock.unlock();
			cemuLog_log(LogType::Force, "[TraceCache] Queued {} functions for prewarming", queuedCount);
			// Wake recompiler threads to start prewarming
			PPCRecompilerState.recompilerWakeCondVar.notify_all();
		}
	}
}
...
{
	// Shutdown trace cache (saves to disk)
	PPCTraceCache::GetInstance().Shutdown();

    // shut down recompiler threads
    s_recompilerThreadStopSignal = true;
    PPCRecompilerState.recompilerWakeCondVar.notify_all(); // wake all threads so they can exit
    for (auto& t : s_recompilerThreads)
    {
        if (t.joinable())
            t.join();
    }
    s_recompilerThreads.clear();
    // clean up queues
