...
namespace coreinit
{
	...
	void OSSchedulerBegin(sint32 numCPUEmulationThreads)
	{
		...
		// Pin scheduler threads to performance cores on hybrid CPUs
		auto& topology = CpuTopology::GetInstance();
		if (topology.IsHybridCPU())
		{
			const auto& pCores = topology.GetPerformanceCores();
			if (pCores.size() >= sSchedulerThreads.size())
			{
				for (size_t i = 0; i < sSchedulerThreads.size(); i++)
				{
					// Pin each scheduler thread to a different P-core
					// Use every other P-core to avoid hyperthreading siblings
					uint32_t coreIndex = (i * 2) % pCores.size();
					if (CpuTopology::SetThreadAffinityToCore(g_schedulerThreadHandles[i], pCores[coreIndex]))
					{
						cemuLog_log(LogType::Force, "Scheduler thread {} pinned to P-core {} (logical processor {})",
							i, coreIndex, pCores[coreIndex]);
					}
				}
			}
		}
