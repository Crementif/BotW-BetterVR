...
private:
	...
	void uploadPages(uint32 firstPage, uint32 lastPagePlusOne)
	{
		...
		// fused copy + hash in single pass to avoid reading memory twice
		const uint8* srcBase = (const uint8*)memory_getPointerFromPhysicalOffset(uploadRangeBegin);
		uint8* dstBase = s_pageUploadBuffer.data();
		for (uint32 i = 0; i < numPages; i++)
		{
			m_pageInfo[firstPage + i].hash = copyAndHashPage(dstBase + i * CACHE_PAGE_SIZE, srcBase + i * CACHE_PAGE_SIZE);
		}
	...
	sint32 uploadPageWithStreamoutFiltered(uint32 pageIndex)
	{
		...
		// fused copy + hash in single pass
		m_pageInfo[pageIndex].hash = copyAndHashPage(pageCopy, (const uint8*)memory_getPointerFromPhysicalOffset(pageBase));
	...
	// Fused copy + hash operation - copies data while computing hash in single pass
	// This avoids reading the same memory twice (once for memcpy, once for hash)
	static uint64 copyAndHashPage_scalar(uint8* dst, const uint8* src)
	{
		static const uint64 k0 = 0x55F23EAD;
		static const uint64 k1 = 0x185FDC6D;
		static const uint64 k2 = 0xF7431F49;
		...
		uint64 h0 = 0, h1 = 0, h2 = 0, h3 = 0;
		const uint64* srcPtr = (const uint64*)src;
		uint64* dstPtr = (uint64*)dst;
		const uint64* srcEnd = srcPtr + (CACHE_PAGE_SIZE / sizeof(uint64));

#if defined(ARCH_X86_64)
		_mm_prefetch((const char*)(srcPtr + 8), _MM_HINT_T0);
#endif
		...
		while (srcPtr < srcEnd)
		{
#if defined(ARCH_X86_64)
			if (srcPtr + 16 < srcEnd)
				_mm_prefetch((const char*)(srcPtr + 16), _MM_HINT_T0);
#endif
			uint64 v0 = srcPtr[0];
			uint64 v1 = srcPtr[1];
			uint64 v2 = srcPtr[2];
			...
			h0 = std::rotr(h0, 7); h1 = std::rotr(h1, 7);
			h2 = std::rotr(h2, 7); h3 = std::rotr(h3, 7);
...
#if defined(ARCH_X86_64)
	// AVX2 optimized version - uses 256-bit loads/stores for faster memory copy
	ATTRIBUTE_AVX2
	static uint64 copyAndHashPage_AVX2(uint8* dst, const uint8* src)
	{
		static const uint64 k0 = 0x55F23EAD;
		static const uint64 k1 = 0x185FDC6D;
		static const uint64 k2 = 0xF7431F49;
		...
		const __m256i* srcPtr256 = (const __m256i*)src;
		__m256i* dstPtr256 = (__m256i*)dst;
		const __m256i* srcEnd256 = srcPtr256 + (CACHE_PAGE_SIZE / 32);
		const uint64* hashPtr = (const uint64*)src;

		// Prefetch ahead
		_mm_prefetch((const char*)(srcPtr256 + 2), _MM_HINT_T0);
		_mm_prefetch((const char*)(srcPtr256 + 4), _MM_HINT_T0);

		while (srcPtr256 < srcEnd256)
		{
			_mm_prefetch((const char*)(srcPtr256 + 6), _MM_HINT_T0);

			// Load and store 64 bytes using AVX2
			__m256i data0 = _mm256_loadu_si256(srcPtr256);
			__m256i data1 = _mm256_loadu_si256(srcPtr256 + 1);
			_mm256_storeu_si256(dstPtr256, data0);
			_mm256_storeu_si256(dstPtr256 + 1, data1);

			// Hash 8 uint64 values (unrolled)
			for (int i = 0; i < 2; i++)
			{
				uint64 v0 = hashPtr[0], v1 = hashPtr[1];
				uint64 v2 = hashPtr[2], v3 = hashPtr[3];
	...
	// Dispatcher - selects AVX2 or scalar at runtime
	static uint64 copyAndHashPage(uint8* dst, const uint8* src)
	{
		cemu_assert_debug((CACHE_PAGE_SIZE % 32) == 0);
#if defined(ARCH_X86_64)
		if (g_CPUFeatures.x86.avx2)
			return copyAndHashPage_AVX2(dst, src);
#endif
		return copyAndHashPage_scalar(dst, src);
	}
